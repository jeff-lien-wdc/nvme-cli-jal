...
static const char * const telemetry_stat_id_str[] = {
	...
	[TELEMETRY_STAT_ID_SUC]		= "DRAM Uncorrectable Count",
...
typedef struct telemetry_stats_desc {
	__le16 id;
	__u8 info;
	__u8 ns_info;
	__le16 size;
	__le16 rsvd1;
	__u8 data[];
} telemetry_stats_desc_t;

typedef struct telemetry_event_desc {
	__u8 class;
	__le16 id;
	__u8 size;
	__u8 data[];
} telemetry_event_desc_t;
...
void print_stats_desc(telemetry_stats_desc_t *stat_desc);
