...
#include "wdc-nvme.h"
#include "wdc-utils.h"
#include "plugins/ocp/ocp-hardware-component-log.h"
...
static int wdc_vs_hw_rev_log(int argc, char **argv, struct command *command,
		struct plugin *plugin)
{
	/* The wdc plugin command for hw revision is not up to date  */
	/* call the ocp plugin hw component log command here instead */
	return ocp_hwcomp_log(argc, argv, command, plugin);
}
